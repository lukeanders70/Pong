...
function Camera:draw(tex, quad, x, y, parent, rotation, scaleX, scaleY, offsetX, offsetY, originX, originY)
	if parent then
		love.graphics.drawScaled(
			tex,
			quad,
			x + self.x_offest + parent.x + (offsetX or 0),
			y + self.y_offset + parent.y + (offsetY or 0),
			rotation,
			scaleX,
			scaleY,
			originX,
